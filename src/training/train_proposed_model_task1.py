...
LABELED_FRACTION = 1.0  # Task1은 전부 labeled로 사용 (순수 supervised + SSL/DG)
...
def train_one_epoch_my_model_ssl_dg(
    ...
    # 2) unlabeled 배치: VAE + SSL + DG 만 (있으면)
...
def run_task_my_model_task1(task_name: str):
    """
    Task1: Tomato Dry vs Cut (box, LOPO).
    MyModel + VAE + SSL + (형식적인) DG.
    """
    set_seed(SEED)
    project_root, plantsounds_root = get_project_paths()
    data_root = plantsounds_root

    print(f"\n===== MyModel Task1: {task_name} =====")
    ...
    exp_dir = project_root / "experiments" / "task1_my_model"
...
def main():
    # 필요하면 다른 Task1 variant 이름을 넣어서 실행해도 됨
    task_list = [
