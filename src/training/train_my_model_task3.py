# src/training/train_my_model_task3.py
...
# Task3는 box vs greenhouse 같은 도메인 정보를 활용해서
# DG(DANN)를 쓸 수 있지만, 여기서는 구조만 유지 (alpha=0)
ALPHA_DG = 0.1
...
def _compute_pos_weight_from_dataset(
    ...
    if n_pos == 0 or n_neg == 0:
        print("[WARN] (Task3) labeled subset 클래스 분포가 한쪽으로 치우쳐 pos_weight 사용 불가.")
        return None

    pos_weight_val = n_neg / n_pos
    print(
        f"    [Task3 labeled balance] neg={n_neg}, pos={n_pos}, "
...
def train_one_epoch_my_model_semi(
    ...
    """
    Task3(greenhouse generalization)에 대한 MyModel 세미-슈퍼바이즈드 학습 1 epoch.
    """
    model.train()
    total_loss = 0.0
    total_sup_samples = 0
...
def run_task_my_model_task3(task_name: str = "task3_tomato_vs_greenhouse"):
    """
    Task3: Tomato drought(온실) vs Greenhouse background noise 에 대해
           MyModel(VAE+SSL 구조)로 LOPO-CV 실험 수행.
    """
    set_seed(SEED)
    project_root, plantsounds_root = get_project_paths()
    data_root = plantsounds_root

    print(f"\n===== MyModel Task3: {task_name} =====")
    ...
    num_classes = 2  # tomato vs greenhouse noise

    fold_results: List[Tuple[float, float]] = []

    exp_dir = project_root / "experiments" / "task3_my_model"
...
def main():
    run_task_my_model_task3("task3_tomato_vs_greenhouse")
